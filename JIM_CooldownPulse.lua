...
do -- Change the following values to customize your version of Cooldown Pulse without having to wait for JIM
    ...
    function Pulse_LoadVariables()
        ...
        if (not Pulse_MinimapButton_show) then
            Pulse_MinimapButton:Hide()
        end
    end
...
do -- FUNCTIONAL VARS AND GLOBAL STRINGS
    ...
    local localization = {
        ...
        deDE = {
            -- German translations courtesy of Bartleby @ Curse-Gaming
            "Letzte Aktion",
            "Kann eine Aktion nicht wiederholen. Aktiviere %s in %s den Optionen."
        }
    }
    localization =
        localization[GetLocale()] or
        {
            "Recent Action",
            "Cannot repeat an action.  Enable %s in %s options."
        }

    PULSE_RECENT_ACTION, PULSE_NO_HISTORY = unpack(localization)
    ...
    function Pulse_AddEffect(effect, start, duration, enable)
        ...
        if (start ~= 0) then
			...
			We may still want to keep tabs on it for a second cooldown.
                ...
                if
                    (not Pulse_CDT.disabled[name]) and -- User criterion: Excludes
                        ((duration >= Pulse_minCooldown) and (duration <= Pulse_maxCooldown) or -- User criterion: min / max duration
                            Pulse_CDT.enabled[name])
                 then -- User criterion: Includes
                    ...
                    for i = table.getn(Pulse_inCooldown), 1, -1 do
                        if Pulse_inCooldown[i] == effect then
                            table.remove(Pulse_inCooldown, i)
                        end
                    end
                    assert("Pulse_AddEffect", name, start, duration, enable)

                    local _existingIndex, insertionIndex =
                        SearchAscendingList(
                        effect.cooledAt,
    ...
    function Pulse_CheckForCooldown(effect)
        ...
        if (effect.slot) then -- this is an item ...
            ...
            if (effect.bag) then -- ... that is in a bag.
                ...
                start, duration, enable = GetContainerItemCooldown(effect.bag, effect.slot)
            else -- ... that is equipped.
                assert("Pulse_CheckForCooldown", "of an equipped ITEM.")

                start, duration, enable = GetInventoryItemCooldown("player", effect.slot)
            end
        ...
        if not start or not duration then
            Pulse_WaitOff(effect) -- remove unactual effect
            return false
        end
        ...
        if (Pulse_history) then
            ...
            if (effect) then
                ...
                elseif (effect.bag) then
                    ...
                    UseInventoryItem(effect.slot)
                end
            end
        else
            error(string.format(PULSE_NO_HISTORY, PULSE_RECENT_ACTION, BINDING_HEADER_PULSE), 1)
        end
    end
...
do -- HOOKED API
    ...
    function SpellCast_Attempt(spell, castingFunction, ...)
        ...
        if (successful) and (not spell.cooledAt) then
            Pulse_CheckForCooldown(spell)
        end
        return successful
    end
    ...
    function SpellCast_End()
        if (event == "SPELLCAST_STOP") and not Pulse_CheckForCooldown(SpellCast_inProgress) then
            Pulse_WaitOn(SpellCast_inProgress)
        end
        Pulse_End()
    end
end -- HOOKED API
...
do -- CALLED BY THE XML
    ...
    function Pulse_Animate()
        ...
        while (checkEffect) do -- unless there's an expiry, this loop only repeats once per Animate
            if not checkEffect.cooledAt then
                ...
                elseif (waited >= 0) then
                    ...
                    if (Pulse_cooldownSound) then
                        PlaySoundFile(Pulse_cooldownSound)
                    end
                        ...
                        table.sort(
                            ...
                            function(v1, v2)
                                if (v1 and v2) then
                                    ...
                                    return (not priority[v2.name]) or
                                        (priority[v1.name] and (priority[v1.name] < priority[v2.name]))
                                else
                                    return v1
                                end
                            end
                    ...
                    if (Pulse_CheckForCooldown(checkEffect) == false) then
                        Pulse_WaitOn(checkEffect)
                    end
                    break
                else
                    break
                end -- incoming animation check
        ...
        if (anim) then
            ...
            Pulse_Image:SetAlpha(
                (anim < Pulse_lubTime) and (anim * Pulse_maxAlpha / Pulse_lubTime) or -- pulse fade in
                    (anim < Pulse_dubTime) and (Pulse_dubTime - anim) * Pulse_maxAlpha / (Pulse_dubTime - Pulse_lubTime) or -- pulse fade out
                    0 -- animation over
            )
